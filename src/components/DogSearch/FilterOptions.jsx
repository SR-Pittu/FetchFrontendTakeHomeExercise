...
const FilterOptions = ({
  ...
  const [selectedReactSelectValue, setSelectedReactSelectValue] = useState(
    initialBreeds.map((breed) => ({ value: breed, label: breed }))
  );
  ...
  useEffect(() => {
    const newSelectedOptions = selectedBreeds.map((breed) => ({
      value: breed,
      label: breed,
    }));
  ...
  const handleRemoveSelectedBreed = (breedToRemove) => {
    setSelectedBreeds((prevBreeds) =>
      prevBreeds.filter((breed) => breed !== breedToRemove)
    );
    setSelectedReactSelectValue((prev) =>
      prev.filter((option) => option.value !== breedToRemove)
    );
  };
  ...
  return (
    <div className="filter-options-container">
      <h2 className="filter-description">
        Narrow down your search for the perfect dog.
      </h2>
